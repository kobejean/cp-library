...
data:
  ...
  bundledCode: "'''\n\u257A\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\
    ...
    \ isubset_deconv(A: list[int], B: list[int], N: int, mod: int) -> list[int]:\n\
    \    Z = (N+1)*(M:=1<<N)\n    Ar, Br, P = [0]*Z, [0]*Z, popcnts(N)\n    for i,\
    \ p in enumerate(P): Ar[p<<N|i], Br[p<<N|i] = A[i], B[i]\n    isubset_deconv_ranked(Ar,\
    \ Br, N, Z, M, mod)\n    for i, p in enumerate(P): A[i] = Ar[p<<N|i] % mod\n \
    \   return A\n"
  code: "import cp_library.__header__\nfrom cp_library.bit.popcnts_fn import popcnts\n\
    import cp_library.math.__header__\nimport cp_library.math.conv.__header__\nfrom\
    \ cp_library.math.conv.mod.isubset_deconv_ranked_fn import isubset_deconv_ranked\n\
    import cp_library.math.conv.mod.__header__\n\ndef isubset_deconv(A: list[int],\
  ...
  verifiedWith: []
documentation_of: cp_library/math/conv/mod/isubset_deconv_fn.py
layout: document
redirect_from:
- /library/cp_library/math/conv/mod/isubset_deconv_fn.py
- /library/cp_library/math/conv/mod/isubset_deconv_fn.py.html
