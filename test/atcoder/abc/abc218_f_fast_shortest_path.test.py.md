...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://atcoder.jp/contests/abc218/tasks/abc218_f\n\
    ...
    \        deg, Ea, Ua, Va, La, i = u32a(N), u32a(M := len(U)), u32a(M), u32a(M),\
    \ u32a(N), 0\n        for u in U: deg[u] += 1\n        for u in range(N): La[u],\
    \ i = i, i+deg[u]\n        Ra = La[:]\n        for e in range(M):\n          \
    \  i = Ra[u := U[e]]\n            Ua[i], Va[i], Ea[i], Ra[u] = u, V[e], e, i+1\n\
    \        super().__init__(N, M, U, V, deg, La, Ra, Ua, Va, Ea)\n\n    def scc(G)\
    \ -> Iterator[list[int]]:\n        \"\"\"\n        Finds strongly connected sccs\
    \ in directed graph using Tarjan's algorithm.\n        Returns sccs in topological\
    \ order.\n        \"\"\"\n        Ra, tin, low, on_stack, I, time = G.Ra, i32a(N\
    \ := G.N, -1), u32a(N), u8a(N), G.La[:], 0\n        order, stack, sccs, L = elist(N),\
...
layout: document
redirect_from:
- /verify/test/atcoder/abc/abc218_f_fast_shortest_path.test.py
- /verify/test/atcoder/abc/abc218_f_fast_shortest_path.test.py.html
