...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/stirling_number_of_the_first_kind\n\
    ...
    \        if n>>i&1:res,t = conv(res,[-t,1]),t+1\n    if signed: return [mint(res[k])\
    \ for k in range(n+1)]\n    else: return [mint(-res[k] if (k^n)&1 else res[k])\
  ...
  verifiedWith: []
documentation_of: test/stirling_number_of_the_first_kind.test.py
layout: document
redirect_from:
- /verify/test/stirling_number_of_the_first_kind.test.py
- /verify/test/stirling_number_of_the_first_kind.test.py.html
