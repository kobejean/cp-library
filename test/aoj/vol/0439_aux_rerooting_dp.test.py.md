...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/problems/0439\n\
    ...
    \ T.Ra[v]\n        T.Ua[i], T.Va[i], T.Wa[i], T.Ua[j], T.Va[j], T.Wa[j] = u, v,\
    ...
    \ not st or t != a: st.append(a)\n                    V.append(l); post.append(T.add(l,\
    \ a))\n                st.append(v)\n            l = st.pop()\n            while\
    \ st: V.append(l); post.append(T.add(l, l := st.pop()))\n            V.append(l)\n\
    \            yield c, V, post\n            while V:\n                u = V.pop()\n\
    \                T.Ra[u] = T.La[u]\n                if T.vis: T.vis[u] = 0\n \
  ...
  code: "# verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/problems/0439\n\
    \nfrom cp_library.alg.dp.min2_fn import min2\nfrom cp_library.alg.tree.fast.aux_tree_cls\
    \ import AuxTree\n\ndef main():\n    N = read(int)\n    C = read(list[-1, N])\n\
    \    T = read(AuxTree[N])\n\n    def edge(s, i, p, u, c):\n        return T.Wa[i]\
    \ if C[u] == c else s+T.Wa[i]\n    \n    ans = T.rerooting_dp(C, N, min2, edge)\n\
...
layout: document
redirect_from:
- /verify/test/aoj/vol/0439_aux_rerooting_dp.test.py
- /verify/test/aoj/vol/0439_aux_rerooting_dp.test.py.html
