...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/courses/library/5/GRL/3/GRL_3_A\n\
    ...
    \ return hash(tuple(que.q[que.h:]))\n\ndef cut_vertices(G: GraphBase, s: Union[int,list,None]\
    \ = None):\n    '''\n    Find cut vertices in an undirected graph using DFS edge\
    ...
    \ stdin: 'IO'; stdout: 'IO'\n    __slots__ = 'f', 'file', 'B', 'O', 'V', 'S',\
    \ 'l', 'p', 'char', 'sz', 'st', 'ist', 'writable', 'encoding', 'errors'\n    def\
  ...
  code: "# verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/courses/library/5/GRL/3/GRL_3_A\n\
    \ndef main():\n    N, M = read()\n    G = read(Graph[N,M,0])\n    ap = cut_vertices(G)\n\
    \    ans = [v for v,b in enumerate(ap) if b]\n    if ans:\n        write(*ans,\
    \ sep='\\n')\n\nfrom cp_library.alg.graph.csr.snippets.cut_vertices_fn import\
  ...
  verifiedWith: []
documentation_of: test/aoj/grl/grl_3_a_cut_vertices_snippet_fn.test.py
layout: document
redirect_from:
- /verify/test/aoj/grl/grl_3_a_cut_vertices_snippet_fn.test.py
- /verify/test/aoj/grl/grl_3_a_cut_vertices_snippet_fn.test.py.html
