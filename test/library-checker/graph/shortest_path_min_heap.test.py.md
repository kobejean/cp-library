...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/shortest_path\n\
    ...
    \ iterable: Iterable = None): self.data = list(iterable) if iterable else [];\
    \ heapify(self.data)\n    def pop(self): return heappop(self.data)\n    def push(self,\
    \ item: _T): heappush(self.data, item)\n    def pushpop(self, item: _T): return\
    \ heappushpop(self.data, item)\n    def replace(self, item: _T): return heapreplace(self.data,\
...
layout: document
redirect_from:
- /verify/test/library-checker/graph/shortest_path_min_heap.test.py
- /verify/test/library-checker/graph/shortest_path_min_heap.test.py.html
