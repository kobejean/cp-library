...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/cycle_detection\n\
    ...
    \ N: int, U: list[int], V: list[int]):\n        deg, Ea, Ua, Va, La, i = u32a(N),\
    \ u32a(M := len(U)), u32a(M), u32a(M), u32a(N), 0\n        for u in U: deg[u]\
    \ += 1\n        for u in range(N): La[u], i = i, i+deg[u]\n        Ra = La[:]\n\
    \        for e in range(M):\n            i = Ra[u := U[e]]\n            Ua[i],\
    \ Va[i], Ea[i], Ra[u] = u, V[e], e, i+1\n        super().__init__(N, M, U, V,\
    ...
    \     Ra, tin, low, on_stack, I, time = G.Ra, i32a(N := G.N, -1), u32a(N), u8a(N),\
...
layout: document
redirect_from:
- /verify/test/library-checker/graph/cycle_detection.test.py
- /verify/test/library-checker/graph/cycle_detection.test.py.html
