...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/tree_path_composite_sum\n\
    ...
    \         for i in range(M): u, v, a, b = io.readints(); U[i], V[i], W[i], X[i]\
    ...
    \                    return cls(N, U, V, W, X)\n        elif len(w) == 3:\n  \
    \          w, x, y = w\n            def parse(io: IOBase):\n                U,\
    \ V, W, X, Y = u32f(M), u32f(M), [0]*M, [0]*M, [0]*M\n                for i in\
    \ range(M): U[i], V[i], W[i], X[i], Y[i] = u(io), v(io), w(io), x(io), y(io)\n\
    \                return cls(N, U, V, W, X, Y)\n        else:\n            w, x,\
    \ y, z = w\n            def parse(io: IOBase):\n                U, V, W, X, Y,\
    \ Z = u32f(M), u32f(M), [0]*M, [0]*M, [0]*M, [0]*M\n                for i in range(M):\
  ...
  code: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/tree_path_composite_sum\n\
    ...
    \ import pack_dec\nfrom cp_library.alg.tree.csr.tree_weighted_meta_cls import\
  ...
  verifiedWith: []
documentation_of: test/library-checker/tree/tree_path_composite_sum.test.py
layout: document
redirect_from:
- /verify/test/library-checker/tree/tree_path_composite_sum.test.py
- /verify/test/library-checker/tree/tree_path_composite_sum.test.py.html
