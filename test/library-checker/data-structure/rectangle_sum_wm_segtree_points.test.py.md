...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/rectangle_sum\n\
    ...
    \ i in range(Q):\n        append(str(wm.prod_rect(*map(int, input().split()))));\
  ...
  code: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/rectangle_sum\n\
    ...
    \ map(int, input().split())\n    wm = WMSegTreePoints(add, 0, X, Y, W)\n    for\
    \ i in range(Q):\n        append(str(wm.prod_rect(*map(int, input().split()))));\
    \ append('\\n')\n    os.write(1, sb.build().encode())\n\nfrom cp_library.ds.wavelet.wm_segtree_points_cls\
    \ import WMSegTreePoints\nimport sys,os\nfrom __pypy__ import builders # type:\
    \ ignore\nsb = builders.StringBuilder()\nappend = sb.append\ndef input(): return\
...
layout: document
redirect_from:
- /verify/test/library-checker/data-structure/rectangle_sum_wm_segtree_points.test.py
- /verify/test/library-checker/data-structure/rectangle_sum_wm_segtree_points.test.py.html
