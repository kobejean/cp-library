...
data:
  ...
  code: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/range_kth_smallest\n\
    ...
    \ in range(Q):\n        l, r, k = sys.stdin.readline().split()\n        append(str(wm.kth(int(k),\
    \ int(l), int(r)))); append('\\n')\n    os.write(1, sb.build().encode())\n\nfrom\
    \ cp_library.ds.wavelet.wm_compressed_cls import WMCompressed\nimport os\nimport\
    \ sys\nfrom __pypy__ import builders\nsb = builders.StringBuilder()\nappend =\
    \ sb.append\n\nif __name__ == \"__main__\":\n    main()"
  dependsOn:
...
layout: document
redirect_from:
- /verify/test/library-checker/data-structure/range_kth_smallest_wm_compressed.test.py
- /verify/test/library-checker/data-structure/range_kth_smallest_wm_compressed.test.py.html
