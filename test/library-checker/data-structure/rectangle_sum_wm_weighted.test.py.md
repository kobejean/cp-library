...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/rectangle_sum\n\
    ...
    \ distinct=True); icoord_compress_multi(D, U, Yn)\n    Y, W = [0]*N, [0]*N\n \
    \   for i,j in enumerate(Xn): Y[j], W[j] = Yn[i], Wn[i]\n    wm = WMWeighted(Y,\
    \ W)\n    for i in range(Q): append(str(wm.sum_rect(L[i], D[i], R[i], U[i])));\
  ...
  code: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/rectangle_sum\n\
    ...
    \   for i,j in enumerate(Xn): Y[j], W[j] = Yn[i], Wn[i]\n    wm = WMWeighted(Y,\
    \ W)\n    for i in range(Q): append(str(wm.sum_rect(L[i], D[i], R[i], U[i])));\
    \ append('\\n')\n    os.write(1, sb.build().encode())\n\nfrom cp_library.ds.wavelet.wm_weighted_points_cls\
    \ import WMWeightedPoints\nfrom cp_library.ds.wavelet.wm_weighted_cls import WMWeighted\n\
  ...
  verifiedWith: []
documentation_of: test/library-checker/data-structure/rectangle_sum_wm_weighted.test.py
layout: document
redirect_from:
- /verify/test/library-checker/data-structure/rectangle_sum_wm_weighted.test.py
- /verify/test/library-checker/data-structure/rectangle_sum_wm_weighted.test.py.html
